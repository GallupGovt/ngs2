...
# step 2: create the list of eligibles for the experiment
...
python misc/prep_eligibles.py \
    ...
    -m $6/flash_mob_complete_data_23oct2017.csv \
    -o $6/flash_mob_${DATE}_$3.csv

# step 3: determine if invites are to be randomized
# this uses command-line input to determine if the csv created in step 2 for
# the email blast through qualtrics should be randomized into two groups to run
# an incentive experiment.
if [ $5 == 'exp' ]
    then
        # step 3.1: randomize invites
        # this takes the csv from step 2 and randomly splits it into `control`
        # and `treatment` groups. these are then the two lists to go to qualtrics
        # for an email push.
        ...
        # step 4: create the sms messages list
        # this takes the csv output from step 2 and creates a csv that will be used to
        # send sms messages via the twilio api. it drops records that have opted-out of
        # getting messages as well as cleans up or drops phone number entries. it is
        # used to push sms messages 15 minutes before experiment time.
        cd $6
